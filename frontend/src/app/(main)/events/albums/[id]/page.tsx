...
import { ArrowLeft, Upload, QrCode, Image, Trash2, Lock, Globe } from 'lucide-react';
...
export default function AlbumDetailPage() {
  ...
  const handleShowQR = async () => {
    try {
      const res = await api.get(`/albums/${id}/qr`);
  ...
  const handleDeleteAlbum = async () => {
    if (!album) return;
    if (!confirm(`Delete album "${album.name}"? This will remove all media inside it and cannot be undone.`)) return;
  ...
  // Can delete the album: ADMIN, or the event creator
  const eventCreatorId = (album.event as any)?.creatorId;
  const canDeleteAlbum =
    user?.role === 'ADMIN' || (eventCreatorId && user?.id === eventCreatorId);

  // Can upload to this album: ADMIN or PHOTOGRAPHER
  const canUpload = user && ['ADMIN', 'PHOTOGRAPHER'].includes(user.role);

  return (
    <div className="space-y-6">
      <Link href={`/events/${album.eventId}`} className="flex items-center gap-2 text-slate-400 hover:text-slate-200">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          {album.description && <p className="text-slate-400 mt-1">{album.description}</p>}
        ...
        <div className="flex gap-2 flex-shrink-0">
          <button onClick={handleShowQR} className="btn-secondary text-sm">
      ...
      {showQR && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setShowQR(false)}>
          <div className="card p-6 text-center max-w-sm" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">Share Album via QR</h3>
            {qrCode && <img src={qrCode} alt="QR Code" className="mx-auto w-48 h-48" />}
            <p className="text-sm text-slate-400 mt-3">{album.name}</p>
            <button onClick={() => setShowQR(false)} className="btn-secondary mt-4 w-full justify-center">
              Close
            </button>
          </div>
        </div>
